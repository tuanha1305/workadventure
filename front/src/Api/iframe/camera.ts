...
export class WorkAdventureCameraCommands extends IframeApiContribution<WorkAdventureCameraCommands> {
     ...
     */
    followPlayer(smooth: boolean = false): void {
        sendToWorkadventure({
     ...
     */
    set(x: number, y: number, width?: number, height?: number, lock: boolean = false, smooth: boolean = false): void {
        sendToWorkadventure({
            type: "cameraSet",
     ...
     *
     * @returns {Subject<WasCameraUpdatedEvent>} Subject of camera update event
     */
